...
function initializePage() {
    ...
    function adjustMainContent() {
        ...
        mainContent.style.marginLeft = leftPanel.style.display === 'none' ? '0' : leftPanelWidth;
        mainContent.style.width = `calc(100% - ${leftPanelWidth})`; // Ensure main content is centered

        const hoverArea = document.getElementById('hoverArea');
        hoverArea.style.left = leftPanel.style.display === 'none' ? '25%' : leftPanelWidth;
...
function displayImage() {
    ...
    const prevButton = document.getElementById('prevButton');
    prevButton.style.transition = 'none'; // Disable transition temporarily
    ...
    image.onload = () => {
        image.style.display = 'block'; // Show the image after it has fully loaded
    };

    setTimeout(() => {
        prevButton.style.transition = 'left 0.3s ease'; // Re-enable transition
    }, 10);
}
...
function togglePlayPause() {
    ...
    console.log(playIcon, pauseIcon); // Check if icons are found
    if (intervalId) {
